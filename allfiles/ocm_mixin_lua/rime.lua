...
--      - lua_filter@reverse_lookup_filter        -- （lua資料夾）（關）依地球拼音為候選項加上帶調拼音的註釋
--      - lua_filter@myfilter                     -- （lua資料夾）（關）把 charset_comment_filter 和 reverse_lookup_filter 註釋串在一起，如：CJK(hǎo)
--
...
--      - lua_filter@symbols_mark_filter          --（lua資料夾）（關，但 mix_cf2_cfp_smf_filter 有用到某元件，部分開啟） 候選項註釋符號、音標等屬性之提示碼(comment)（用 opencc 可實現，但無法合併其他提示碼(comment)，改用 Lua 來實現）
--      - lua_filter@missing_mark_filter          --（lua資料夾）（關） 補上標點符號因直上和 opencc 衝突沒附註之選項
--      - lua_filter@array30_comment_filter       --（lua資料夾）（關） 遮屏提示碼，開關（simplify_comment）（遇到「`」不遮屏）
--      - lua_filter@array30_nil_filter           --（onion-array30） 行列30空碼'⎔'轉成不輸出任何符號，符合原生。後來移至「=」「=」反查用。
--      - lua_filter@array30_spaceup_filter       --（lua資料夾）（關） 行列30開關一二碼按空格後，是否直上或可能有選單。
--      - lua_filter@en_sort_filter               --（easy_en_super和其掛接）如同英漢字典一樣排序，候選項重新排序。開關（en_sort）
--
--      - ＊合併兩個以上函數：
--      - lua_filter@mix30_nil_comment_filter     --（lua資料夾）（關） 合併 array30_nil_filter 和 array30_comment_filter，兩個 lua filter 太耗效能。
...
--      - lua_processor@endspace                  --（lua資料夾）（關） （hangeul 和 hangeul2set） 韓語（非英語等）空格鍵後添加" "（該方式無法記錄到用戶詞典，故使用原生程式另外方式達成該項功能）
--      - lua_processor@ascii_punct_change        --（bopomo_onionplus_2和3） 注音非 ascii_mode 時 ascii_punct 轉換後按 '<' 和 '>' 能輸出 ',' 和 '.'
--      - lua_processor@array30up                 --（lua資料夾）（關） 行列30三四碼字按空格直接上屏
--      - lua_processor@array30up_zy              --（lua資料夾）（關） 行列30注音反查 Return 和 space 上屏修正
--
--      = 以下針對「編碼有用到空白鍵」方案，如：注音一聲，去除空白上屏產生莫名之空格 =
--      - lua_processor@s2r_ss                    --（lua資料夾）（關） 注音掛接 t2_translator 空白上屏產生莫名空格去除（只有開頭 ^'/ 才作用，比下條目更精簡，少了 is_composing 限定）
--      - lua_processor@s2r_s                     --（lua資料夾）（關） 注音掛接 t2_translator 空白上屏產生莫名空格去除（只有開頭 ^'/ 才作用）
--      - lua_processor@s2r                       --（lua資料夾）（關） 注音掛接 t2_translator 空白上屏產生莫名空格去除（ mixin(1,2,4)和 plus 用）
--      - lua_processor@s2r_e_u                   --（lua資料夾）（關） 注音掛接 t2_translator 空白上屏產生莫名空格去除（只針對 email 和 url ）
--      - lua_processor@s2r_most                  --（lua資料夾）（關） 注音掛接 t2_translator 空白上屏產生莫名空格去除（ mixin(1,2,4)和 plus 用，精簡寫法）
--      - lua_processor@s2r_mixin3                --（lua資料夾）（關） 注音掛接 t2_translator 空白上屏產生莫名空格去除（ mixin3 (特殊正則)專用）
...
--]]
function en_sort_filter(input, env)
...
function t_translator(input, seg)
  if (string.match(input, "`")~=nil) then
    ...
    if (input == "`fh") then
      ...
      yield(Candidate("date", seg.start, seg._end, string.gsub("民國 "..min_guo(os.date("%Y")).." 年 "..os.date("%m").." 月 "..os.date("%d").." 日", "([^%d])0", "%1"), "〔年*月*日〕"))
      ...
      yield(Candidate("date", seg.start, seg._end, "民國 "..min_guo(os.date("%Y")).." 年 "..os.date("%m").." 月 "..os.date("%d").." 日", "〔年*月*日〕"))
    ...
    if (input == "`fn") then
      ...
      yield(Candidate("date", seg.start, seg._end, os.date("%Y-%m-%dT%H:%M+08:00"), "〔ISO 8601/RFC 3339〕 ~i"))
    ...
    if (input == "`fni") then
      yield(Candidate("date", seg.start, seg._end, os.date("%Y-%m-%dT%H:%M+08:00"), "〔當地時  ISO 8601/RFC 3339〕"))
      yield(Candidate("date", seg.start, seg._end, os.date("%Y%m%dT%H%M+08"), "〔當地時  ISO 8601/RFC 3339〕"))
      yield(Candidate("date", seg.start, seg._end, os.date("%Y-%m-%d-%H-%M ") .. timezone_out1()[1], "〔當地時  時區〕"))
      yield(Candidate("date", seg.start, seg._end, os.date("%Y-%m-%d-%H-%M ") .. timezone_out1()[2], "〔當地時  時區〕"))
    ...
    if (input == "`fnh") then
      ...
      yield(Candidate("date", seg.start, seg._end, string.gsub("民國 "..min_guo(os.date("%Y")).." 年 "..os.date("%m").." 月 "..os.date("%d").." 日 "..os.date("%H 點 %M 分"), "([^%d])0", "%1"), "〔年月日*時:分〕"))
      ...
      yield(Candidate("date", seg.start, seg._end, "民國 "..min_guo(os.date("%Y")).." 年 "..os.date("%m").." 月 "..os.date("%d").." 日 "..os.date("%H 點 %M 分"), "〔年月日*時:分〕"))
    ...
    if (input == "`ft") then
      ...
      yield(Candidate("date", seg.start, seg._end, os.date("%Y-%m-%dT%H:%M:%S+08:00"), "〔ISO 8601/RFC 3339〕 ~i"))
    ...
    if (input == "`fti") then
      yield(Candidate("date", seg.start, seg._end, os.date("%Y-%m-%dT%H:%M:%S+08:00"), "〔當地時  ISO 8601/RFC 3339〕"))
      yield(Candidate("date", seg.start, seg._end, os.date("%Y%m%dT%H%M%S+08"), "〔當地時  ISO 8601/RFC 3339〕"))
      yield(Candidate("date", seg.start, seg._end, os.date("%Y-%m-%d-%H-%M-%S ") .. timezone_out1()[1], "〔當地時  時區〕"))
      yield(Candidate("date", seg.start, seg._end, os.date("%Y-%m-%d-%H-%M-%S ") .. timezone_out1()[2], "〔當地時  時區〕"))
    ...
    if (input == "`fth") then
      ...
      yield(Candidate("date", seg.start, seg._end, string.gsub("民國 "..min_guo(os.date("%Y")).." 年 "..os.date("%m").." 月 "..os.date("%d").." 日 "..os.date("%H 點 %M 分 %S 秒"), "([^%d])0", "%1"), "〔年月日*時:分:秒〕"))
      ...
      yield(Candidate("date", seg.start, seg._end, "民國 "..min_guo(os.date("%Y")).." 年 "..os.date("%m").." 月 "..os.date("%d").." 日 "..os.date("%H 點 %M 分 %S 秒"), "〔年月日*時:分:秒〕"))
    ...
    if (input == "`ymh") then
      ...
      yield(Candidate("date", seg.start, seg._end, string.gsub("民國 "..min_guo(os.date("%Y")).." 年 "..os.date("%m").." 月", "([^%d])0", "%1"), "〔年*月〕"))
      yield(Candidate("date", seg.start, seg._end, "民國"..fullshape_number(min_guo(os.date("%Y"))).."年"..fullshape_number(string.gsub(os.date("%m"), "0([%d])", "%1")).."月", "〔年月〕"))
      yield(Candidate("date", seg.start, seg._end, "民國"..min_guo(os.date("%Y")).."年"..os.date("%m").."月", "〔年月〕"))
      yield(Candidate("date", seg.start, seg._end, "民國 "..min_guo(os.date("%Y")).." 年 "..os.date("%m").." 月", "〔年*月〕"))
