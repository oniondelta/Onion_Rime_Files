...
local function init(env)
  ...
  local prefix = config:get_string(env.namespace2 .. "/prefix")
  -- local suffix = config:get_string(env.namespace2 .. "/suffix")
  local pattern = config:get_string("recognizer/patterns/" .. env.namespace2) or config:get_string("recognizer2/patterns/" .. env.namespace2)
  -- local pattern = "';[-a-z0-9,./; ]*'?$"  -- 測試用
...
local function segmentor(segs, env)
  ...
  -- seg.tags = { "all_bpm" = true }  -- 錯誤格式，無法作用！
  ...
  -- seg.prompt = "〖 TEST: " .. env.namespace2 .. " 〗 正則：".. env.pattern .. " «end» "  -- "  尾綴：" .. env.suffix .. 
  ...
  -- seg2.tags = Set({env.namespace2})
  -- seg2.prompt = "〖 TEST: " .. env.namespace2 .. " 〗" .. " startpos: " .. startpos .. " endpos: " .. endpos .. " cofmpos: " .. cofmpos .. " segpos: " .. segpos .. " segs.input:len(): " .. segs.input:len() .. " caret_pos: " .. caret_pos .. " #c_input: " .. #c_input .. " «end» "
  -- segs:add_segment(seg2)
  -- -- segs:trim()

  --- 決定是否終止後面附加 segmentor（打tag）
  return false  -- 停止處理，可如此設置，但 lua_segmentor@mount_allbpm 須放在 affix_segmentor@all_bpm 之後。
  -- return true  -- 不停止處理，補打完 tag，讓後面 affix_segmentor@all_bpm 可去處理。
end
