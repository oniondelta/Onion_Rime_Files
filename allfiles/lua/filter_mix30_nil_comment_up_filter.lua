...
local function mix30_nil_comment_up_filter(inp, env)
  ...
  for cand in inp:iter() do
    if (string.match(cand.text, '^⎔%d$' )) then
      ...
      if (s_up) then
        if (string.match(find_prefix, "^[a-z.,/;][a-z.,/;]?[a-z.,/;']?[a-z.,/;']?[i']?$" )) then
          yield( s_c_f_p_s and change_comment(cand,"") or cand )
         elseif(string.match(find_prefix, "^[a-z,./;][a-z,./;]? $" )) and (not string.match(cand.comment, '▪' )) then
          yield(cand)  --空格後，該字編碼不含「▪」直接上屏。
        elseif (string.match(find_prefix, "^a[k,] $" )) or (string.match(find_prefix, "^lr $" )) or (string.match(find_prefix, "^ol $" )) or (string.match(find_prefix, "^q[ka] $" )) or (string.match(find_prefix, "^%.b $" )) or (string.match(find_prefix, "^/%. $" )) or (string.match(find_prefix, "^pe $" )) then
          yield(cand)  --同時含有「▫」和「▪」編碼的字，空格後，該字某編碼下可直接上屏，某編碼下不能直接上屏。
        elseif (string.match(find_prefix, "^sf $" )) and (string.match(cand.text, '毋' )) then
          yield(cand)  --同時含有「▫」和「▪」編碼的字，但該編碼不只一個字。
        elseif (string.match(find_prefix, "^lb $" )) and (string.match(cand.text, '及' )) then
          yield(cand)  --同時含有「▫」和「▪」編碼的字，但該編碼不只一個字。
        elseif (string.match(find_prefix, "^ou $" )) and (string.match(cand.text, '○' )) then
          yield(cand)  --同時含有「▫」和「▪」編碼的字，但該編碼不只一個字。
  ...
  -- if (not s_c_f_p_s) or (string.match(find_prefix, "`" )) then
    for cand in input:iter() do
      ...
      if (string.match(cand.text, '^⎔%d$' )) then
        ...
        if (s_up) then
          if (string.match(find_prefix, "^[a-z,./;][a-z,./;]? $" )) and (not string.match(cand.comment, '▪' )) then
            yield(cand)
          -- elseif (string.match(find_prefix, "^www%..*$")) then
          --   yield(cand)
          elseif (string.match(find_prefix, "^[a-z.,/;][a-z.,/;]?[a-z.,/;']?[a-z.,/;']?[i']?$" )) or (string.match(find_prefix, "^a[k,] $" )) or (string.match(find_prefix, "^lr $" )) or (string.match(find_prefix, "^ol $" )) or (string.match(find_prefix, "^q[ka] $" )) or (string.match(find_prefix, "^%.b $" )) or (string.match(find_prefix, "^/%. $" )) or (string.match(find_prefix, "^pe $" )) then
            yield(cand)
          elseif (string.match(find_prefix, "^sf $" )) and (string.match(cand.text, '毋' )) then
            yield(cand)
          elseif (string.match(find_prefix, "^lb $" )) and (string.match(cand.text, '及' )) then
