...
local function convert_japan_translator(input, seg)
  ...
  if tag_abc and (c~=nil) then
    local hw = halfwidth_kata_t(revise_t(c))
    yield(Candidate("jp", seg.start, seg._end, revise_t(c), "〔羅馬字〕"))
    yield(Candidate("jp", seg.start, seg._end, fullshape_t(revise_t(c)), "〔全形羅馬字〕"))
...
-- local function p_convert_japan_translator(input, seg, env)
  ...
  if tag_jp and (c~=nil) then
    ...
    local roma_f = Candidate("jp", seg.start, seg._end, fullshape_t(revise_t(c)), "〔全形羅馬字〕")
