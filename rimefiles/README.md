...
「詞典」：只有詞句。
「字詞典」：有單字也有詞句。
「主字詞典」：設定檔中連結的字詞典，該檔內可連結引入其他字詞典。
  ps：官方 dict 並無以上如此區分，皆為引入字詞的 .dict.yaml 檔，由於習慣單字和詞句分開放置，故此區分。
  ps：如接入多個不同之字詞典，這邊習慣以 .extended.dict.yaml 結尾命名「主字詞典」。
...
├── Mount_ocm_encoder.custom.yaml              
├── Mount_ocm_encoder.extended.dict.yaml       
├── Mount_ocm_encoder.schema.yaml              
├── README.md                                  〈本說明文件〉
...
├── bo_mixin_kr_hnc.dict.yaml                  〈注音 mixin 系列方案〉「字典」：韓文 HNC 鍵位。
├── bo_mixin_la.dict.yaml                      〈注音 mixin 系列方案〉「字典」：拉丁。含英文字母、音標、帶調字母等。
...
├── bopomo_onion_double.schema.yaml            〈雙拼注音 方案〉「設定檔」：可簡拼。
├── bopomo_onion_double_phrase.txt             〈雙拼注音 方案〉「用戶短語」
├── bopomo_onion_double_t2.schema.yaml         〈雙拼注音 方案〉「設定檔」：不簡拼。
├── bopomo_onion_phrase.txt                    〈純注音 方案〉「用戶短語」
├── bopomo_onion_symbols.yaml                  〈純注音 方案〉「引入檔」：標點符號。
...
├── element_bopomo.yaml                        〈注音 mixin 和 plus 方案〉「引入檔」：方案設置程式碼。
├── element_bopomo_double.yaml                 〈雙拼注音 方案〉「引入檔」：方案設置程式碼。
├── element_ocm.yaml                           
...
├── hangeul.dict.yaml                          （未引用）〈掛載用 韓文形碼 方案〉「字典」：韓文之諺文。音碼類改用 HNC 版。
├── hangeul.extended.dict.yaml                 （未引用）〈掛載用 韓文形碼 方案〉「主字詞典」：音碼類改用 HNC 版。
├── hangeul.schema.yaml                        （未引用）〈掛載用 韓文形碼 方案〉「設定檔」：音碼類改用 HNC 版。
├── hangeul_hanja.dict.yaml                    （未引用）〈掛載用 韓文形碼 方案〉「字典」：韓文之漢字。音碼類改用 HNC 版。
├── hangeul_hnc.dict.yaml                      〈掛載用 韓文 HNC 方案〉「字典」：漢文之諺文。注音方案掛載。
├── hangeul_hnc.extended.dict.yaml             〈掛載用 韓文 HNC 方案〉「主字詞典」：注音方案掛載。
├── hangeul_hnc.schema.yaml                    〈掛載用 韓文 HNC 方案〉「設定檔」：注音方案掛載。
├── hangeul_hnc_hanja.dict.yaml                〈掛載用 韓文 HNC 方案〉「字典」：韓文漢字。注音方案掛載。
├── hangeul_hnc_phrase.txt                     〈掛載用 韓文 HNC 方案〉「用戶短語」：注音方案掛載。
├── hangeul_phrase.txt                         （未引用）〈掛載用 韓文形碼 方案〉「用戶短語」：音碼類改用 HNC 版。
├── jpnin1.dict.yaml                           〈掛載用 日文 方案〉「字典」：含假名和單漢字。
├── jpnin1.extended.dict.yaml                  〈掛載用 日文 方案〉「主字詞典」
├── jpnin1.schema.yaml                         〈掛載用 日文 方案〉「設定檔」
├── jpnin1_hw.dict.yaml                        〈掛載用 日文 方案〉「字典」：半形片假名。
...
├── onion-array10-symbols-prefix.dict.yaml     〈行列 10 方案〉「字典」：須輸入前綴之符號。
├── onion-array10-symbols.dict.yaml            〈行列 10 方案〉「字典」：直出之符號。
...
├── onion-array30-phrases.dict.yaml            〈行列 30 方案〉「詞典」：多字詞句。
├── onion-array30-shortcode.dict.yaml          〈行列 30 方案〉「字典」：簡碼。
├── onion-array30-special.dict.yaml            〈行列 30 方案〉「字典」：特別碼。按空格才出現或上屏之編碼。
...
├── phrases.cht.dict.yaml                      （未引用）〈注音相關方案〉「詞典」：未標註拼音之中文詞句。破音字因素，未有方案引用。逐步標註拼音移至 phrases.chtp.dict.yaml。
├── phrases.cht_en_w.dict.yaml                 〈各種相關方案〉「詞典」：中英混合詞彙。
├── phrases.chtp.dict.yaml                     〈注音相關方案〉「詞典」：已標註拼音。
├── phrases.cyr_all.dict.yaml                  〈各種相關方案〉「詞典」：西里爾（俄文）。
├── phrases.en_l_w.dict.yaml                   〈各種相關方案〉「詞典」：英文小寫。
├── phrases.en_o_w.dict.yaml                   〈各種相關方案〉「詞典」：英文名品牌和含有標點等特殊。
├── phrases.en_u_w.dict.yaml                   〈各種相關方案〉「詞典」：英文大寫和首字母大寫。
├── phrases.fs_all.dict.yaml                   〈各種相關方案〉「詞典」：全形字母。
├── phrases.gr_all.dict.yaml                   〈各種相關方案〉「詞典」：希臘文。
├── phrases.jp_hk.dict.yaml                    〈日語相關方案〉「詞典」：日文純假名小。
├── phrases.jp_hk_more.dict.yaml               〈日語相關方案〉「詞典」：日文純假名大。比 phrases.jp_hk.dict.yaml 詞彙量更多。
├── phrases.jp_hkk.dict.yaml                   〈日語相關方案〉「詞典」：日文。假名和漢字皆含。
├── phrases.jp_hkkreduce.dict.yaml             〈注音 mixin 系列方案〉「詞典」：日文純假名。減除中文字詞典中已有字詞彙。
├── phrases.jp_hkkseg.dict.yaml                〈掛載用 日文 方案〉「詞典」：日文標註發音，不包含純假名詞句，純假名放置在 phrases.jp_hk_more.dict.yaml 或 phrases.jp_hk.dict.yaml。只在 jpnin1.schema.yaml 該設定檔使用。
├── phrases.jp_hkmoreup_w.dict.yaml            （未引用）「詞典」：日文。使與漢字同發音之純假名字串可顯示於選項中，後期增加更大日文詞典，已無效益。
├── phrases.jp_hkup_w.dict.yaml                （未引用）「詞典」：日文。使與漢字同發音之純假名字串可顯示於選項中，後期增加更大日文詞典，已無效益。
├── phrases.kr.dict.yaml                       〈韓文相關方案〉「詞典」：韓文小詞典。
├── phrases.kr_more.dict.yaml                  〈韓文相關方案〉「詞典」：韓文大詞典。比 phrases.kr.dict.yaml 詞彙量更多。
├── phrases.la_eu_w.dict.yaml                  〈拉丁相關方案〉「詞典」：非純 26 英文字母。如：café、rosé 等歐洲語言詞彙。
├── phrases.la_py_w.dict.yaml                  〈拉丁相關方案〉「詞典」：中文拼音。
├── phrases.ocmtc_essay_mc.dict.yaml           
├── punct_bopomo.yaml                          〈注音相關方案〉「引入檔」：標點符號。
├── punct_bopomo_double.yaml                   〈雙拼注音相關方案〉「引入檔」：標點符號。
├── punct_ocm.yaml                             
├── punct_ovff.dict.yaml                       
├── punct_ovff.schema.yaml                     
├── rime.lua                                   〈含 lua 功能之方案〉「 lua 程式碼檔」：引入用，主要程式文件在 lua 資料夾中。
├── space.dict.yaml                            〈注音相關方案〉「字典」：一些掛載方案需有空格編碼，使主方案〈注音 plus space 方案〉可出空格，獨立字典檔好開關。對其他大千注音方案，在某些操作下，掛載方案上屏可少一鍵。
├── space_f.dict.yaml                          〈掛載用 全形字母 方案〉「字典」：同上，但為全形空白。
